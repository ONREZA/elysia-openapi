...
- configurable OpenAPI `3.0.x`, `3.1.x`, and `3.2.x` output via `openapiVersion`
- OpenAPI 3.1/3.2 nullable schema normalization
- OpenAPI 3.2 document metadata, `QUERY`, additional HTTP operations, and streaming media metadata
...
- WebSocket routes excluded from OpenAPI paths
...
For OpenAPI 3.1 and 3.2, `documentation` can include top-level fields such as
`jsonSchemaDialect` and `webhooks`. OpenAPI 3.2 also accepts `$self`, named
servers, nested tags, reusable `components.mediaTypes`, streaming
`itemSchema`, and the other typed 3.2 metadata exported by this package.
...
List of methods to exclude from documentation. WebSocket routes are always
skipped. In OpenAPI 3.2, `QUERY` is emitted as a normal Path Item operation and
recognized extended HTTP methods are emitted under `additionalOperations`.
...
For OpenAPI 3.1 and 3.2 the target is `draft-2020-12`. For OpenAPI 3.0 the
target is `openapi-3.0`. `context.io` / `context.typeMode` is `input` for
requests and `output` for responses.
