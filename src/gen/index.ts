...
const loadTypeScript = (projectRoot: string): typeof import('typescript') => {
	...
	if (!runtimeRequire)
		throw new Error(
			'[@onreza/elysia-openapi/gen] `fromTypes` import type resolution requires CommonJS require or module.createRequire'
		)
	...
	try {
		...
		throw new Error(
			'[@onreza/elysia-openapi/gen] `fromTypes` import type resolution requires `typescript`. Install it with: bun add -d typescript'
		)
	}
}
